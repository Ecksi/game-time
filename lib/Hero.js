  ...
  dropSquished(currentBurger) {
    ...
    if (currentBurger[0].smushCount === 2 && currentBurger[0].y + 10 < 580) {
      ...
      if (burgerOnNextPlatform && nextPlatform.l != 8) {
        nextPlatform = platformArray.find(i => burgerOnNextPlatform.x > i.x && (i.x + i.w) > burgerOnNextPlatform.x && i.l > nextPlatform.l);
        let burgerOnNextNextPlatform = this.burgerLayer.burgerLayers.find(i => i.y + i.h < nextPlatform.y + nextPlatform.h && nextPlatform.y < i.y + i.h && i.x === burgerOnNextPlatform.x)

        gameBurgerFall.play();
        burgerOnNextPlatform.y = nextPlatform.y - 10;

        if (burgerOnNextNextPlatform && nextPlatform.l != 8) {
          nextPlatform = platformArray.find(i => burgerOnNextNextPlatform.x > i.x && (i.x + i.w) > burgerOnNextNextPlatform.x && i.l > nextPlatform.l);
          let burgerOnNextNextNextPlatform = this.burgerLayer.burgerLayers.find(i => i.y + i.h < nextPlatform.y + nextPlatform.h && nextPlatform.y < i.y + i.h && i.x === burgerOnNextNextPlatform.x)

          gameBurgerFall.play();
          burgerOnNextNextPlatform.y = nextPlatform.y - 10;

          if (burgerOnNextNextNextPlatform && nextPlatform.l != 8) {
            nextPlatform = platformArray.find(i => burgerOnNextNextNextPlatform.x > i.x && (i.x + i.w) > burgerOnNextNextNextPlatform.x && i.l > nextPlatform.l);

            gameBurgerFall.play();
            burgerOnNextNextNextPlatform.y = nextPlatform.y - 10;

          } else if (burgerOnNextNextNextPlatform && nextPlatform.l === 8) {
            let currentPlate = this.plates.plates.filter(i => currentBurger[0].x === i.x + 5);
            let currentPlateCount = currentPlate[0].count;
        ...
        } else if (burgerOnNextNextPlatform && nextPlatform.l === 8) {
          let currentPlate = this.plates.plates.filter(i => currentBurger[0].x === i.x + 5);
          let currentPlateCount = currentPlate[0].count;
